...
if($_POST['type'] == "Save")
{
	...
	if(empty($errMsg))
	{
		$pid = $users -> saveUser($id, $name, $username, $password, $email, $phone, $website, $org_info, $publish, $weight);
		if($id > 0)
			$pid = $id;
		$users -> saveImage($pid);
		//$users -> saveMap($pid);
                          ...
                          <?php
							...
							{?>
                          		...
                          		<tr <?php if($counter%2 != 0) echo 'bgcolor="#F7F7F7"'; else echo 'bgcolor="#FFFFFF"'; ?>>
                            		...
                            		<td valign="top"> [ <a href="users.php?type=edit&id=<?= $row['id']?>">Edit</a> | <a href="#" onClick="javascript: if(confirm('This will permanently remove this User from database. Continue?')){ document.location='users.php?type=del&id=<?php echo $row['id']; ?>'; }">
                                    Delete</a> ]</td>

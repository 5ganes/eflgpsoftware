...
class Users
{
 ...
 function validateFrontPassword($id,$pswd)
 {
 	global $conn;
	
  $sql = "SELECT COUNT(*) cnt FROM usergroups WHERE id = '$id' AND password = '$pswd'";
  //echo $sql;
  $result = $conn -> exec($sql);
  $row = $conn -> fetchArray($result);
  if($row['cnt'] > 0)
 ...
 function updateFrontPassword($id,$pswd)
 {
 	global $conn;
 ...
 function saveUser($id, $name, $username, $password, $email, $phone, $website, $org_info, $publish, $weight)
	{
		global $conn;
		$id = cleanQuery($id);
